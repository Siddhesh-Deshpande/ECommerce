...
public class TaskScheduler {
    ...
    public void ReserveMoney() {
        ...
        for(String key : guavaCache.asMap().keySet()) {
            ...
            if(reservePayment.getStatus() == 0) {
                // START SPAN FOR LOCAL PAYMENT RESERVATION
                Span span = this.tracer.nextSpan().name("2pc-local-payment-reserve");
                try (Tracer.SpanInScope ws = this.tracer.withSpan(span.start())) {
                    span.tag("correlationId", key);
                    ...
                    if(user != null) {
                        if(user.getBalance() - reserveAmount < reservePayment.getAmount()) {
                            ...
                            if(fundReserve != null) {
                                ...
                                ReserveFund newReserve = new ReserveFund(reservePayment.getClientid(), reservePayment.getAmount());
                                reserveFundRepository.save(newReserve);
                            }
                            kafkaTemplate.send("coor-service", new PaymentResponse(reservePayment.getCorrelationId(), true));
        ...
        for(String key : keys) { guavaCache.invalidate(key); }
    }
    ...
    public void DeductMoney() {
        ...
        for(String key : guavaCache.asMap().keySet()) {
            ReservePayment reservePayment = guavaCache.asMap().get(key);
            if(reservePayment.getStatus() == 1) {
                // START SPAN FOR LOCAL PAYMENT DEDUCTION
                Span span = this.tracer.nextSpan().name("2pc-local-payment-deduct");
                try (Tracer.SpanInScope ws = this.tracer.withSpan(span.start())) {
                    span.tag("correlationId", key);
                    ...
                    if(user != null && fundReserve != null) {
                        int amountToDeduct = reservePayment.getAmount(); // Using DTO amount for accuracy
                        user.setBalance(user.getBalance() - amountToDeduct);
                        fundReserve.setReserveAmount(fundReserve.getReserveAmount() - amountToDeduct);

                        paymentRepository.save(user);
                        reserveFundRepository.save(fundReserve);

                        logger.info("correlationId: {}, eventType: {}, user_id: {}, new_balance: {}",
                                key, "ChargeMoney", user.getId(), user.getBalance());
