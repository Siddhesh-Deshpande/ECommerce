...
% \usepackage{float}
% \usepackage{stfloats} % Better handling of double-column floats
% %% ---- Standard packages ----
...
% %%
% \author[1]{\fnm{Siddhesh Deshpande} \sur{}}\email{siddhesh.deshpande@iiitb.ac.in}
% \author[2]{\fnm{Madhav Girdhar} \sur{}}\email{madhav.girdhar@iiitb.ac.in}
% \author[3]{\fnm{B Thangaraju} \sur{}}\email{b.thangaraju@iiitb.ac.in}

% %% Uncomment and add co-authors if needed:
% % \author[2]{\fnm{Second} \sur{Author}}\email{author2@institution.edu}

% \affil[1]{\orgname{International Institute of Information Technology Bangalore, Department of Computer Science and Engineering}}
...
% The transition from monolithic architectures to microservices has forced a rethink of how distributed transactions are managed. 
% %
% While monolithic systems rely on a single database to enforce ACID guarantees, the decentralized nature of microservices—where each service manages its own data—elevates cross-service atomicity to a primary architectural challenge.
...
% %
% Although Sagas are often preferred for their high availability, 2PC remains indispensable for high-stakes domains like financial processing and inventory management, where partial commits cannot be tolerated.
...
% %
% Current studies lack a detailed characterization of how event-driven scaling, specifically via KEDA, interacts with 2PC rollback mechanics under high concurrency. 
% %
...
% \begin{enumerate}
%   \item A production-grade 2PC scheduler implemented over Apache Kafka using Spring Boot 3, fully instrumented with OpenTelemetry for distributed tracing.
...
% Section~\ref{sec:evaluation} discusses the experimental results; and
...
% We opted for the Two-Phase Commit (2PC) protocol over the ubiquitous Saga pattern to satisfy the strict atomicity requirements inherent in e-commerce inventory reservations. While Sagas offer superior availability and avoid the blocking overhead typical of 2PC, they rely on eventual consistency and compensating actions. 

% In high-concurrency e-commerce environments, this reliance on eventual consistency can lead to critical failures, such as overselling, if inventory is exhausted before a compensation can be triggered. By contrast, 2PC ensures synchronous atomicity during the prepare phase. By locking inventory reservations upfront, the system guarantees that once a transaction moves to the commit phase, the necessary resources are secured. Although this approach introduces a known latency penalty, the trade-off is necessary for workflows where data integrity and the prevention of overselling are non-negotiable business constraints.
...
% \begin{center}
%   \captionof{table}{Kafka topic topology and message flow classification.}
...
% To monitor the system, Prometheus is configured to scrape the Spring Actuator
...
% %
% For visualization, Grafana includes four pre-configured dashboards detailing JVM
...
% Table 3 details the latency percentiles observed across the different load levels.
% %
...
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.85\linewidth]{Images/NegativeFlow.png}
...
% \textbf{Git Repository Link}: https://github.com/Siddhesh-Deshpande/ECommerce
...
% \begin{enumerate}
%   \item \textbf{Persistence and Crash Recovery}: Replacing the in-memory Guava cache with a distributed store, such as Redis, to guarantee that transaction states survive coordinator pod restarts.
%   \item \textbf{Comparative Consistency Models}: Conducting a rigorous, side-by-side evaluation to benchmark our 2PC approach against a purely asynchronous Saga choreography pattern.
%   \item \textbf{Dynamic Scaling Thresholds}: Exploring machine learning techniques to adaptively adjust KEDA scaling triggers in response to live traffic and resource demands.
%   \item \textbf{Observability Extensions}: Automating trace-context propagation for cache eviction events to eliminate visibility gaps during decentralized rollbacks.
% \end{enumerate}
